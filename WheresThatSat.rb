...
catalog.keys.each do |satellite|
	search(format '"%s"', satellite) do |tweet|
		...
		reply_text = format "#USER# When you mentioned %s, it was located above %.4f %s %.4f %s at an altitude of %.2f km.", satellite, latitude.abs, latitude >= 0 ? "N" : "S", longitude.abs, longitude >= 0 ? "E" : "W", altitude
